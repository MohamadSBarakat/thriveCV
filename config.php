...
public function getProfileUserById($idUser) {
        ...
        $prof=$requete_prepare->fetchObject('ProfileC');
        return $prof;
    }
...
/* ------------------------------------------------------------------------------------------------------*/
/* //////////////////////////////////////  getAllUserByActivite   [ 8 ]   ///////////////////////////////*/
/* ------------------------------------------------------------------------------------------------------*/

public function getAllUserByActivite ($pattern) {        
    $requete_prepare = $this->connexion->prepare(
        "SELECT P.idUser, P.firstName, P.vedioT, P.permiTra, P.idAct, A.id ,A.actFr  
         FROM profileC P
    INNER JOIN activite A ON A.id = P.idAct  
    WHERE  A.actFr  LIKE  :pattern");  

     $requete_prepare -> execute(
        array('pattern' => "%$pattern%"));

     $listUs = $requete_prepare->fetchAll(PDO::FETCH_CLASS, 'ProfileC');
     return $listUs;
}
...
/* //////////////////////////////////////  getAllUserByActivite   [ 8 ]   ///////////////////////////////*/
/* ------------------------------------------------------------------------------------------------------*/
public function getAllUserByAct() { 
    ...
    INNER JOIN activite A ON A.id = P.idAct");  
     ...
     $listU = $requete_prepare->fetchAll(PDO::FETCH_CLASS, 'ProfileC');
     return $listU;
}
...
/* ------------------------------------------------------------------------------------------------------*/
 ...
 class ProfileC {
    ...
    public $actFr;
        ...
        public function getUserAcivitie() { return $this->actFr; }
